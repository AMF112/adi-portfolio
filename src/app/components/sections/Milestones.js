    ...
    <section id="milestones" className="py-16 md:py-20 lg:py-24 mt-10 w-full font-sans">
      <div className="mb-12 md:mb-16 space-y-3">
        <h2 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-black tracking-tighter text-white uppercase leading-tight">
          Journey & <span className="bg-clip-text text-transparent bg-gradient-to-r from-purple-400 via-purple-500 to-purple-600">Milestones</span>
        </h2>
      ...
      <div className="relative max-w-5xl">
        <div className="absolute left-[15px] md:left-[25%] top-2 bottom-2 w-[2px] -translate-x-1/2 bg-white/[0.10] pointer-events-none z-10" />
          ...
          <AnimatePresence mode="wait">
            ...
            >
              {data[activeTab].map((item, index) => (
                <div
                  key={`${activeTab}-${index}`}
                  className="group/item relative flex flex-col md:flex-row md:items-start justify-between gap-3 md:gap-0 pb-14 last:pb-0 pl-11 md:pl-0 transition-all duration-300"
                >
                  <div className="md:w-[23%] shrink-0 flex flex-col items-start md:items-end justify-start gap-1 md:text-right pt-1 md:pt-4 md:pr-10">
                    <span className="text-base sm:text-lg md:text-xl font-black text-slate-400 tracking-wide transition-colors duration-300 group-hover/item:text-purple-400">
                      {item.date}
                    </span>
                    <p className="text-xs font-extrabold text-slate-500 uppercase tracking-wider transition-colors duration-300 group-hover/item:text-slate-300">
                      {item.org}
                    </p>
                  </div>
                  ...
                  <div className="absolute left-[15px] md:left-[25%] top-[26px] md:top-6 -translate-x-1/2 -translate-y-1/2 z-30 flex items-center justify-center pointer-events-none">
                    <div className="relative w-2.5 h-2.5 rounded-full bg-neutral-950 border-2 border-white/30 group-hover/item:border-purple-400 group-hover/item:bg-purple-500 transition-all duration-300 group-hover/item:scale-125 group-hover/item:shadow-[0_0_12px_rgba(168,85,247,0.6)] pointer-events-auto" />
                  </div>

                  {/* Right Column: Logo & Content */}
                  <div className="flex-1 flex items-start gap-5 sm:gap-7 min-w-0 pl-4 md:pl-16 pt-2 md:pt-1.5">
                    <div className="shrink-0 w-14 h-14 sm:w-16 sm:h-16 md:w-20 md:h-20 relative transition-transform duration-500 group-hover/item:scale-[1.03]">
                      <Image
                        src={item.logo}
                        alt={item.org}
                        fill
                        className="object-contain p-1 brightness-90 contrast-125 group-hover/item:brightness-110 transition-all duration-500"
                      />
                    </div>

                    <div className="flex-1 min-w-0 pt-2 md:pt-4">
                      <h3 className="text-lg sm:text-xl md:text-2xl font-black text-slate-300 tracking-tight leading-snug transition-colors duration-300 group-hover/item:text-white">
                        {item.title}
                      </h3>
                    </div>
                  </div>
